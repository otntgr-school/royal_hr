...
(function (window, document, $) {
  ...
  var select = $('.select2'),
    ...
    selectAsync = $(".select2-async")

  let language = {
    inputTooShort: ({ input, minimum, params }) =>
    {
      return `${minimum}-с дээш утгатай байж хайна`
    },
    noResults: () =>
    {
  ...
  select.each(function () {
    ...
    $this.select2({
      ...
      closeOnSelect: !isMultiple //  хэрвээ multiple байвал auto аар хаагддагыг болиулсан
    });
  });
  ...
  selectBullets.each(function () {
    ...
    $this.select2({
      ...
      width: '100%',
      dropdownParent: $this.parent(),
      templateResult: bulletsFormat,
      templateSelection: bulletsFormat,
      escapeMarkup: function (es) {
        return es;
      }
    });
  ...
  function bulletsFormat(opt)
  {
    var originalOption = opt.element;
    var $opt = `<span class="bullet bullet-sm me-50" style="background-color: ${$(originalOption).data("ccolor")}"></span> ${opt.text}`
    return $opt;
  }
  ...
  selectAsync.wrap('<div class="position-relative"></div>').select2(
    {
      dropdownAutoWidth: true,
      dropdownParent: selectAsync.parent(),
      width: '100%',
  ...
  $('#select2InModal').on('shown.bs.modal', function () {
    selectInModal.select2({
      ...
      dropdownParent: selectInModal.parent(),
      language,
    });
  });
